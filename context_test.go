...
func TestManualTransactions_ContextCancellation(t *testing.T) {
	db, cleanup := createDB(t)
	defer cleanup()

	ctx := context.Background()

	t.Run("Begin with canceled context does not open a transaction", func(t *testing.T) {
		node, err := db.Begin(canceledCtx(), true)
		require.Error(t, err)
		require.True(t, errors.Is(err, context.Canceled))
		require.Nil(t, node)

		// A fresh transaction must still be obtainable (nothing was left open).
		tx, err := db.Begin(ctx, true)
		require.NoError(t, err)
		require.NoError(t, tx.Rollback())
	})

	t.Run("Commit with canceled context does not confirm writes", func(t *testing.T) {
		tx, err := db.Begin(ctx, true)
		require.NoError(t, err)

		require.NoError(t, tx.Save(ctx, &User{ID: 77, Name: "committed"}))

		err = tx.Commit(canceledCtx())
		require.Error(t, err)
		require.True(t, errors.Is(err, context.Canceled))

		// Rollback is required by the caller because Commit did not run.
		require.NoError(t, tx.Rollback())

		var u User
		gErr := db.One(ctx, "ID", 77, &u)
		require.Equal(t, ErrNotFound, gErr, "canceled Commit must not persist")
	})

	t.Run("Rollback still works after cancellation", func(t *testing.T) {
		tx, err := db.Begin(ctx, true)
		require.NoError(t, err)

		require.NoError(t, tx.Save(ctx, &User{ID: 88, Name: "rollback-after-cancel"}))

		// Cleanup must remain available after the caller's operation context is canceled.
		...
		var u User
		gErr := db.One(ctx, "ID", 88, &u)
		require.Equal(t, ErrNotFound, gErr)
	})

	t.Run("successful Commit is not converted to error by later cancellation", func(t *testing.T) {
		// To prove this we commit under a non-canceled context, then cancel a
		// separate context afterward. The committed write must remain visible.
		tx, err := db.Begin(ctx, true)
		require.NoError(t, err)

		require.NoError(t, tx.Save(ctx, &User{ID: 99, Name: "stays"}))

		commitCtx, cancel := context.WithCancel(context.Background())
		err = tx.Commit(commitCtx)
		require.NoError(t, err)
		cancel() // cancel after successful commit

		var u User
		require.NoError(t, db.One(ctx, "ID", 99, &u))
		require.Equal(t, "stays", u.Name)
	})
}
